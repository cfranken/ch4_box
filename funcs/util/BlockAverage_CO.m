...
%%% =----------------------------------------------------------------------
%%% = NOTES
%%% =  ( 1): Gives a pressure weighted moving average.
%%% =----------------------------------------------------------------------
%%% = INPUTS
...
function [ xO, yO, E ] = BlockAverage_CO(St,x,y,p,nD,varargin)
...
for i = 1:length(xO)
    ...
    if sum(ind) < 1
        ...
        % Convert error to an error on the mean
        E(i) = E(i) / sqrt( sum(ind) - 1 );
    end
end
% Remove NaNs
