...
yLims_n2o_lab     = cell(size(yLims_n2o));
yLims_c2h6_lab    = cell(size(yLims_c2h6));
...
for i = 1:length(yLims_n2o);     yLims_n2o_lab{i}     = sprintf('%4.0f',yLims_n2o(i));     end
for i = 1:length(yLims_c2h6);    yLims_c2h6_lab{i}    = sprintf('%4.0f',yLims_c2h6(i));    end
...
%%% Plot the observations
h = figure('position',[0 0 700 900]);
% CH4
ax(1) = subplot(5,1,1);p = get(ax(1),'pos');
set(ax(1),'pos',[p(1),p(2)-0.02,p(3)-.04,p(4)+0.04])
set(gca,pOpts{:},'YTick',yLims_ch4)
box on
set(ax(1),'yaxislocation','left','YTick',yLims_ch4,'YTickLabel',yLims_ch4_lab)
ylabel(ax(1),title_ch4,tOpts{:})
...
set(gca,'XTickLabel',{})
% delta13C
ax(2) = subplot(5,1,2);p = get(ax(2),'pos');
set(ax(2),'pos',[p(1),p(2)-0.02,p(3)-.04,p(4)+0.04])
set(gca,pOpts{:},'YTick',yLims_ch4c13)
box on
set(ax(2),'yaxislocation','right','YTick',yLims_ch4c13,'YTickLabel',yLims_ch4c13_lab)
ylabel(ax(2),title_ch4c13,tOpts{:})
...
set(gca,'XTickLabel',{})
% MCF
ax(3) = subplot(5,1,3);p = get(ax(3),'pos');
set(ax(3),'pos',[p(1),p(2)-0.02,p(3)-.04,p(4)+0.04])
semilogy([NaN,NaN],[NaN,NaN]);
set(gca,pOpts{:},'YTick',yLims_Lmcf,'YTickLabel',yLims_Lmcf_lab)
hold on;
box on
...
set(gca,'XTickLabel',{})
% Ethane
ax(4) = subplot(5,1,4);p = get(ax(4),'pos');
set(ax(4),'pos',[p(1),p(2)-0.02,p(3)-.04,p(4)+0.04])
set(gca,pOpts{:},'YTick',yLims_c2h6)
box on
set(ax(4),'yaxislocation','right','YTick',yLims_c2h6,'YTickLabel',yLims_c2h6_lab)
ylabel(ax(4),title_c2h6,tOpts{:})
hold on
if add_error
    for i = 1:length(St)
        yV = data.nh_c2h6(i); yE = data.nh_c2h6_err(i);
        if ~isnan(yV) && ~isnan(yE)
            plot([St(i),St(i)],[yV-yE,yV+yE],eOpts{:})
        end
        yV = data.sh_c2h6(i); yE = data.sh_c2h6_err(i);
        if ~isnan(yV) && ~isnan(yE)
            plot([St(i),St(i)],[yV-yE,yV+yE],eOpts{:})
        end
    end
end
plot(St, data.nh_c2h6,onhOpts{:})
plot(St, data.sh_c2h6,oshOpts{:})
plot(St,model.nh_c2h6, nhOpts{:})
plot(St,model.sh_c2h6, shOpts{:})
xlim(xLims)
ylim([yLims_c2h6(1),yLims_c2h6(end)])
datetick('x','yyyy','keeplimits')
set(gca,'XTickLabel',{})
% N2O
ax(5) = subplot(5,1,5);p = get(ax(5),'pos');
set(ax(5),'pos',[p(1),p(2)-0.02,p(3)-.04,p(4)+0.04])
set(gca,pOpts{:},'YTick',yLims_n2o)
box on
set(ax(5),'yaxislocation','left','YTick',yLims_n2o,'YTickLabel',yLims_n2o_lab)
ylabel(ax(5),title_n2o,tOpts{:})
hold on
if add_error
    for i = 1:length(St)
        yV = data.nh_n2o(i); yE = data.nh_n2o_err(i);
        if ~isnan(yV) && ~isnan(yE)
            plot([St(i),St(i)],[yV-yE,yV+yE],eOpts{:})
        end
        yV = data.sh_n2o(i); yE = data.sh_n2o_err(i);
        if ~isnan(yV) && ~isnan(yE)
            plot([St(i),St(i)],[yV-yE,yV+yE],eOpts{:})
        end
    end
end
plot(St, data.nh_n2o,onhOpts{:})
plot(St, data.sh_n2o,oshOpts{:})
plot(St,model.nh_n2o, nhOpts{:})
plot(St,model.sh_n2o, shOpts{:})
xlim(xLims)
ylim([yLims_n2o(1),yLims_n2o(end)])
datetick('x','yyyy','keeplimits')
% Save the plot
print(h,printOpts{:},sprintf(baseName,'allSpecies_FULL'))
